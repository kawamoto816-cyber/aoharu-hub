...
//   GET /internal/sales/run?token=...            今日 (JST) の承認分
//   GET /internal/sales/run?token=...&date=YYYY-MM-DD
...
export async function GET(req: Request) {
  ...
  try {
    const result = await runOutreach({ date: p.get("date") ?? undefined, dry: p.get("dry") === "1" });
