...
const COPY: Record<UpgradeReason, { title: string; body: string }> = {
  auth_required: {
    title: "ログインが必要です",
    body: "この機能を使うには登録(ログイン)が必要です。登録は無料で、クレジットカードなしでそのまま無料プランとしてお試しいただけます。",
  },
  quota_exceeded: {
    title: "今月の無料枠を使い切りました",
    body: "無料プランでは、添削レポート・面接の評価レポート・志望理由書の構成案など、各アプリの「結果」を毎月決まった回数まで無料でご利用いただけます。枠は毎月リセットされます。Proプラン(月980円)なら回数制限なくお使いいただけます。",
  },
  no_plan: {
    title: "この機能はProプランからご利用いただけます",
    body: "100点の模範解答、志望理由書・ESの初稿生成、本番形式の模擬面接などはProプラン(月980円)以上でご利用いただけます。いつでも解約できます。",
  },
  payment_issue: {
    title: "お支払いの確認が必要です",
